...
export function conceptPrompt(args: {
  ...
  return [
    ...
    "",
    "EVERY concept must carry the mechanic named above. It is not one option",
    "among several — it is the thing being ripped. A concept that quietly drops",
    "it and finds its own angle has not translated this ad, it has written a",
    "different one, and that is the most common way this task is failed.",
    "",
    `Produce ${args.variantCount} concepts that execute THAT SAME mechanic in`,
    "genuinely different ways: change what gets staged, which proof carries it,",
    "which moment the ad opens on, who is being answered. Two concepts that could",
    "swap headlines are one concept.",
    "",
    "The staging follows from this. If the mechanic works by making something",
    "countable, the image has to let the viewer count it. If it works by",
    "juxtaposition, both sides must be in frame. Do not keep the mechanic in the",
    "copy while the image quietly reverts to a plain packshot.",
    "",
    ...
    "",
    "Open every `visual_prompt` with the staging the idea depends on, stated as a",
    "count and an arrangement — the generator is given a single packshot and will",
    "default to reproducing it unless the first line says otherwise. If the",
    "mechanic needs five units piled up, say five units piled up before anything",
    "else; if it needs one unit alone, say that. An ad whose idea is a bundle and",
    "whose image shows one unit has failed, however beautiful the lighting.",
    "",
